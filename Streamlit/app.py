...
# ============================================================
# 4. LOAD DATA DIRECTLY FROM PROJECT DATASET
# ============================================================
...
def load_data():
    ...
    if not data_path.exists():
        st.error(
            "Insurance dataset was not found. "
            "Please make sure Dataset/insurance_data.csv exists in the repository."
        )
        st.stop()

    insurance_df = pd.read_csv(data_path)

    # Recreate the analytical fields previously generated in Snowflake.
    # The deployed Streamlit app therefore has no Snowflake dependency.
    insurance_df["LOSS_DT"] = pd.to_datetime(
        insurance_df["LOSS_DT"], errors="coerce"
    )
    insurance_df["REPORT_DT"] = pd.to_datetime(
        insurance_df["REPORT_DT"], errors="coerce"
    )

    insurance_df["REPORTING_DELAY_DAYS"] = (
        insurance_df["REPORT_DT"] - insurance_df["LOSS_DT"]
    ).dt.days

    insurance_df["PREMIUM"] = pd.to_numeric(
        insurance_df["PREMIUM_AMOUNT"], errors="coerce"
    )

    insurance_df["CLAIM_TO_PREMIUM_RATIO"] = (
        insurance_df["CLAIM_AMOUNT"]
        / insurance_df["PREMIUM_AMOUNT"].replace(0, np.nan)
    ).round(2)

    insurance_df["AGE_GROUP"] = pd.cut(
        insurance_df["AGE"],
        bins=[24, 29, 39, 49, 59, 64],
        labels=["25-29", "30-39", "40-49", "50-59", "60-64"]
    ).astype("object").fillna("Not Available")

    insurance_df["TENURE_GROUP"] = np.select(
        [
            insurance_df["TENURE"] < 24,
            insurance_df["TENURE"] < 60,
            insurance_df["TENURE"] < 84,
            insurance_df["TENURE"] < 120,
        ],
        [
            "<2 Years",
            "2-4.9 Years",
    ...
    insurance_df["REPORTING_CATEGORY"] = np.where(
        insurance_df["REPORTING_DELAY_DAYS"] == 0,
        "Same Day",
        "Delayed"
    )

    insurance_df["CLAIM_VALUE_CATEGORY"] = np.where(
        insurance_df["CLAIM_AMOUNT"] >= 50000,
        "High Value",
        "Standard"
    )

    insurance_df["LOSS_YEAR"] = insurance_df["LOSS_DT"].dt.year
    insurance_df["LOSS_MONTH"] = insurance_df["LOSS_DT"].dt.month
    insurance_df["LOSS_MONTH_NAME"] = insurance_df["LOSS_DT"].dt.month_name()
...
Raw Insurance Data
        ↓
Excel Data Profiling & SQL Analysis
        ↓
